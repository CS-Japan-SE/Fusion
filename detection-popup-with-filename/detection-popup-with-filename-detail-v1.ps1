# Block Notification with FileName and Detail
...
Function New-ToastNotification {
    Param($MsgSender, $Message, $FilePath, $Info)

    $tempFile = "$env:TEMP\ToastMessage.txt"
    $Contents = @($Info, $FilePath)
...
$Info = "ファイルが悪性と判定されたため隔離しました。`r`nこのファイルが業務で利用する正規のファイルである場合は、情報システム部に連絡をしてください。`r`n連絡先：00-0000-0000"
$Param = parse $args[0]
$FilePath = $Param.File
Invoke-Command –ScriptBlock ${function:New-ToastNotification} –ArgumentList $MsgSender, $Message, $FilePath, $Info