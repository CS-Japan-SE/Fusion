# Block Notification with FileName,Hash and Detail

# Parse Input Filepath and Hash
...
Function New-ToastNotification {
    Param($MsgSender, $Message, $FilePath, $Info, $Hash)

    $tempFile = "$env:TEMP\ToastMessage.txt"
    $Contents = @($Info, $FilePath, $Hash)
    $TextContents = $Contents -join "`r`n"
    Set-Content -Path $tempFile -Value $TextContents -Encoding UTF8
    ...
    # XML format
    [xml]$ToastTemplate = @"
<toast duration="long" activationType="protocol" launch="$tempFile">
<visual>
...
"@
    ...
    Register-ObjectEvent -InputObject $ToastXml -EventName Activated -Action {
...
$Message = "ファイルが隔離されました。詳細はこちらをクリックしてください"
$Info = "ファイルが悪性と判定されたため隔離しました。`r`nこのファイルが業務で利用する正規のファイルである場合は、情報システム部に連絡をしてください。`r`n連絡先：00-0000-0000`r`n`r`n＜隔離されたファイル名とハッシュ値＞"
$Param = parse $args[0]
$FilePath = $Param.File
$Hash = $Param.Hash
Invoke-Command –ScriptBlock ${function:New-ToastNotification} –ArgumentList $MsgSender, $Message, $FilePath, $Info, $Hash